...
type OAuthClient struct {
	...
	Stream         chan *Tweet
	//the current connection to the stream client
...
func getNonce(n int) string {
	// To get a random string with n letters.
...
func signatureBase(httpMethod string, base_uri string, params map[string]string) string {
	...
	buf.WriteString("&")
	buf.WriteString(URLEscape(base_url))
	buf.WriteString("&")
...
func (o *OAuthClient) GetRequestToken(callback string) *RequestToken {
	...
	params := map[string]string{
		...
		"oauth_timestamp":        strconv.Itoa64(time.Second()),
	...
	confirmed, _ := strconv.Atob(token["oauth_callback_confirmed"][0])
	rt := RequestToken{
...
func (o *OAuthClient) GetAccessToken(requestToken *RequestToken, OAuthVerifier string) (*AccessToken, os.Error) {
	...
	params := map[string]string{
		...
		"oauth_signature_method": "HMAC-SHA1",
		"oauth_timestamp":        strconv.Itoa(time.Second()),
		"oauth_consumer_key":     o.ConsumerKey,
		"oauth_version":          "1.0",
	}
...
func (c *oauthStreamClient) connect() (*http.Response, os.Error) {
	...
	resp, err := c.httpClient.AsReponse()
	if err != nil {
		return resp, err
	}
